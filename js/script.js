...
window.onload = function() {
  ...
  function documentActions(e) {
    ...
    if(!targetElement.closest('.menu__item')&& document.querySelectorAll('.menu__item._hover').length>0) {
      for(let i=0; i<document.querySelectorAll('.menu__item._hover').length; i++) {
      document.querySelectorAll('.menu__item._hover')[i].classList.remove('_hover')
      }
    ...
    } else if (!targetElement.closest('.search-form')&& document.querySelectorAll('.search-form.active').length>0) {
      for(let i=0; i<document.querySelectorAll('.search-form.active').length; i++) {
        document.querySelectorAll('.search-form.active')[i].classList.remove('active')
        }
    }
...
let mySwwwiper = new Swiper('.slider-main__body',{
...
let roomsSlider = new Swiper ('.slider-rooms', {
...
let swiperTips = new Swiper(".tips__slider", {
  observer: true,
		observeParents: true,
		slidesPerView: 3,
		spaceBetween: 32,
...
let menu = document.querySelector(".menu__body");
  ...
  document.addEventListener("DOMContentLoaded", function(event) { 
    ...
    if (widthWind <= 750) {
      footerMenu.classList.add("_unit")
    }
    else {
      footerMenu.classList.remove("_unit")
    }
})
...
async function getProducts(button){
  if(!button.classList.contains('_hold')) {
    ...
    const file = "json/mainjson.json";
    let respone = await fetch(file,{
      method:"GET"
    });
    if(respone.ok) {
...
function loadProducts(data) {
  ...
  data.products.forEach(item => {
	...
	`;
			...
			let productTemplatePricesStart = `<div class="item-product__prices prices-item-product">`;
			let productTemplatePricesCurrent = `<div class="prices-item-product__new">Rp ${productPrice}</div>`;
			let productTemplatePricesOld = `<div class="prices-item-product__old">Rp ${productOldPrice}</div>`;
		...
		<div class="item-product__actions actions-product-item">	
    <div class="actions-product-item__button">Add to cart</div>
    <a href="${productShareUrl}"class="actions-product-item__link _icon-share">Share</a>
    <a href="${productLikeUrl}" class="actions-product-item__link _icon-favorite">Like</a>	
		</div>
	`;
...
let createCard = function(product) {
    let listItem = makeElement("article","item-product");
    let label=makeElement("div","item-product__labels");
    listItem.appendChild(label);
    if(product.isSale){
        let labelPrice=makeElement("div","item-product__label_sale");
        listItem.appendChild(labelPrice);
    }
    if(product.isNew){
        let labelNew=makeElement("div","item-product__label_new");
        listItem.appendChild(labelNew)
    }
    let picture=makeElement("img","item-product__image");
    picture.src=product.imgUrl
    listItem.appendChild(picture)
    let bodyItem=makeElement("div","item-product__body");
    listItem.appendChild(bodyItem)
    let bodyContent=makeElement("div","item-product__content");
    listItem.appendChild(bodyContent)
    let itemTitle=makeElement("div","content-item-product__title",product.title)
    listItem.appendChild(itemTitle)

    return listItem
}

let cardList = document.querySelector(".products__items");

for(let i=0; i<4;i++) {
    let cardItem=createCard(productData[i])
